...
SHLIBS_DIRS=vdr/app vdr/master framework/communication vdr/os_proxy vdr/protocols vdr/minion_proxy
SHLIBS_PATHS_COLLON_SEP=$(subst $(space),:,$(SHLIBS_DIRS))
SHLIBS_PATH=$(patsubst %, -L%, $(SHLIBS_DIRS))
SHLIBS= app master communication os_proxy protocols boost_thread boost_system boost_chrono minion_proxy
